...
### 8. NL/Streamlit-authored pipelines share one workload identity per cloud
Each **validated** pipeline gets its own dedicated workload identity at bootstrap (least-privilege, scoped to its exact ServiceAccount + storage). A pipeline authored at runtime through the NL/Streamlit surface has a brand-new ServiceAccount + storage that no pre-created identity trusts, so it assumes a **shared** identity per cloud:
- **AWS:** a shared IRSA role (`multi-cloud-pipelines-irsa`) whose trust is a `StringLike` wildcard over `system:serviceaccount:analytics:*-insights-sa`, policy scoped to the `*-insights-data` bucket convention + Glue + the project SSM namespace.
- **Azure:** a shared user-assigned managed identity (`multi-cloud-pipelines-identity`). An Azure federated-credential subject must be an exact string (no wildcard), so every NL-authored Azure pipeline uses one **fixed** ServiceAccount name (`pipelines-insights-sa`) that the single shared credential trusts; the per-pipeline Terraform binds the shared identity to its own data storage account (Storage Blob Data Contributor).
- **GCP:** a shared service account (`pipelines-insights-sa`). A GKE Workload Identity binding member is exact (no wildcard), so — as on Azure — every NL-authored GCP pipeline uses one **fixed** KSA (`pipelines-insights-sa`) that the single shared binding trusts; the shared SA holds project-level `roles/storage.objectAdmin` over the `*-insights-data` bucket convention.

- **Why:** runtime-created per-pipeline identities would require the deploy credentials to carry IAM/identity-management permissions plus a per-pipeline `terraform apply` of an identity + role binding — broader deploy-time privilege for a demo surface.
- **Production:** the infra agent provisions a per-pipeline identity (trust scoped to that one SA, access scoped to that one storage target) as part of the pipeline's own Terraform, and the deploy identity is granted narrowly-scoped IAM/identity-management permissions.
