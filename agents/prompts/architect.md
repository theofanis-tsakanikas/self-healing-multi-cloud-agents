...
- **Full three-cloud skeleton — never collapse:** Emit ALL THREE `if _CLOUD == "aws":` / `elif _CLOUD == "gcp":` / `elif _CLOUD == "azure":` branches VERBATIM (each with a real body) in the cloud-SDK import block, the idempotency block, AND the credentials block — exactly as `arch_standard_python` shows. The script is cloud-agnostic: only the active cloud's branch runs (conditional imports load only its SDK), so all three are always valid. Do NOT collapse to one branch (collapsing is where the guard gets flattened → CLOUD-GUARD failure, or the import gets dropped → F821 `Undefined name`), and NEVER leave a branch empty/comment-only (a comment-only `elif` body is a `SyntaxError` that dead-loops the self-heal).
...
- **Numeric rule preparation (MANDATORY):** For every quality rule that compares a column numerically (e.g. `> 0.0`, `> 0`, `< 1000`), FIRST cast that column to float in place on its own line — `chunk[col] = chunk[col].astype(float)` — and THEN apply the comparison. Always use `.astype(float)` for this cast; it guarantees a numeric dtype before the comparison.
...
- **Idempotency, Observability, Metrics Emission, Partition Registration:** Follow `arch_standard_python` exactly. In the `sync_partition_metadata` call, inline the schema and table as **string literals** in a plain (non-f) string — do NOT assign `schema`/`table` variables and reference them (the model fills the value in both places → ruff F841 + F541). The schema is the **bare** schema name (the middle segment of `hive.<schema>.<table>`) — NEVER catalog-prefixed. ❌ `'hive.marketing_global'` · ✅ `'marketing_global'`. The objective may print the target fully-qualified; split it and pass only the middle segment.
...
**Adding a missing import (e.g. ruff `F821 Undefined name 'urlparse'`):** you MUST use the dedicated sentinel replacement — it inserts the line after the last import of the file with correct (zero) indentation:
```json
...
**Indentation rule for every patch:** the `new` string MUST carry the exact leading whitespace of the lines it replaces. A replacement that drops the indentation breaks the file. If a single patch would require re-indenting many lines, fix the **one** offending line only.
