...
data "databricks_cluster" "jobs" {
  cluster_name = "multi-cloud-agent-workspace-jobs-cluster"   # bootstrap names it "${workspace_name}-jobs-cluster"
}
...
resource "databricks_job" "pipeline" {
  ...
  task {
    ...
    spark_python_task {
      python_file = "dbfs:/pipelines/pipe_sales_lakehouse/pipe_sales_lakehouse.py"   # CI uploads the script here first
      parameters = [
        "--catalog", var.catalog,
        "--schema", var.schema,
