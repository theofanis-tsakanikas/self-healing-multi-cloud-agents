...
- **uid**: Must be derived from the pipeline name only — never from `project_id` or any session-specific value. Use a lowercase hyphenated slug, e.g. `eu-sales-data-observability`. A stable UID ensures `kubectl apply` updates the existing dashboard instead of creating a duplicate on every run. **Grafana caps the uid at 40 characters** and silently refuses a longer one (`uid too long, max 40 characters` → the dashboard never provisions, Dashboards is empty). When `<slug>-data-observability` would exceed 40 chars (a long pipeline name), fall back to a still-stable, still-name-derived id: the slug prefix + a short deterministic hash of the slug (e.g. `slug[:31]-<sha1(slug)[:8]>`), which stays ≤40 and is identical across runs. (codegen `render_grafana_dashboard` enforces this.)
