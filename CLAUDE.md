...
- Trino partition registration inlines the catalog `hive`, the schema AND the table as **string literals** in a plain (non-f) `cursor.execute` — never `catalog`/`schema`/`table` variables. The model otherwise fills the value into BOTH the assignment and the string, leaving the variable unused (ruff F841) and the f-string placeholder-less (ruff F541). The `sync_partition_metadata` **first arg is the BARE schema** (`'sales_eu'`), never catalog-prefixed (`'hive.sales_eu'`) — the catalog already lives in `hive.system.…`; a prefixed arg fails at RUNTIME with a misleading `Table 'hive.<schema>.<table>' not found`. Guaranteed deterministically (`_fix_sync_partition_schema_arg`, see above).
