...
- **Deterministic generation guarantees:** when a correct output is *mechanically determined* (not a judgement call) yet the LLM still drops it intermittently despite the prompt, resolve it in **Python at generation time** — a guarantee, not a one-off patch. (Since the codegen migration, several of the cases below are SUPERSEDED for the normal path — the artifact is now fully code-owned and the LLM never emits it — but the guards remain in `write_project_file`/`generate_k8s_manifest` as defense for fix-mode and legacy paths: the Lakeview rebuild, the databricks requirements.txt drop, and the `__EMBED_*__` configmap fill. The live cases are the .py-script guards.) Established cases: `write_project_file` auto-injects the cloud-SDK import the script's SDK call requires (F821); `write_project_file` un-doubles f-string braces the model double-braced (`{{…}}` → `{…}`) in a Databricks Spark script (F541 + a corrupted `replaceWhere`); `write_project_file` rebuilds the Databricks Lakeview dashboard JSON from the canonical structure (`_canonical_lakeview_dashboard`) — the LLM reliably substitutes the `_audit` table name but intermittently mangles the nested widget encodings (e.g. nests `color` inside `y.scale`) into invalid JSON, so only the table name is taken from the LLM and the structure is regenerated; `write_project_file` drops `requirements.txt` on Databricks pipelines (detected via the `PIPELINE_PLATFORM` env var set in `main.py` from `target_infra_config.provider`; the LLM emits it empty or pyspark-only and the cluster runtime provides pyspark — `validate_generated_code` likewise never blocks a databricks run on it, with a pyspark-only-content fallback for entry points that don't set the env var); `generate_k8s_manifest` fills `__EMBED_SETUP_TRINO_SQL__` / `__EMBED_MONITORING_SPECS_JSON__` verbatim from disk (truncation/transcription); `write_project_file` **and** `patch_project_file` normalise the `sync_partition_metadata` args to the canonical 3-arg form `('<schema>', '<table>', 'ADD')` (`_fix_sync_partition_schema_arg`) — the catalog `hive` belongs ONLY in the `hive.system.` prefix, never in the args. The LLM injects it two ways, both caught: a **dotted first arg** (`'hive.sales_eu'` → Trino "Table 'hive.sales_eu.<table>' not found") and a **separate 4th catalog arg** (`'hive','schema','table','ADD'` → the mode `'ADD'` is cast to the boolean `case_sensitive` param → "Cannot cast varchar to boolean"). Both fail only at RUNTIME, masked because init-trino's own CALL uses the correct form. Despite explicit ❌/✅ guidance in BOTH the prompt and `python_standards.md` the model still slips (and slips into a NEW shape after each guard — whack-a-mole), so it's a generation guarantee (validator flags both as a safety net for non-guarded paths); `write_project_file` **and** `patch_project_file` normalise source-only column types the LLM copies verbatim from `read_data_schema` into the Trino DDL (`_fix_trino_ddl_types`: `TEXT`/`STRING`/`CHARACTER VARYING`→`VARCHAR`, `DOUBLE PRECISION`→`DOUBLE`) — these are NOT valid Trino types and crash `CREATE TABLE` at runtime with `Unknown type 'X'`; only the unambiguous SYNTACTIC fixes are applied (the semantic choice DOUBLE→DECIMAL-for-money stays the LLM's), and it is **object-storage only** (`not _is_databricks_run()` — Delta legitimately uses `STRING`); validator flags it via the same normaliser; `infra.py` pre-resolves the single per-cloud iac query (no wrong-cloud query). This is distinct from "patching the output" — it injects what is provably required, every time.
- **LLM vs deterministic code — keep the boundary deliberate (not "wherever the LLM happened to fail"):** there are TWO axes of control; keep them distinct.
  ...
  - **(B) Content rewriting** — what the code does to the LLM's output *after* it writes it. Two kinds, and the difference is the whole point:
    ...
    - **Replace** (the LLM step is near-vestigial): the code regenerates the WHOLE artifact and keeps ~nothing from the LLM. The correct end state is to generate it from config and **drop the LLM** for that artifact — **done 2026-06-11** for the Lakeview dashboard and every other fixed-structure artifact (see "the boundary as implemented" below): the LLM call is gone, `agents/codegen.py` renders them.
  - **The rule:** the LLM owns **judgment under variability** (open inputs — an arbitrary source schema, NL business rules, error logs → real pandas/SQL/diagnosis). Deterministic code owns what is **mechanically determined** with one correct answer (naming, structure, wiring, boilerplate). **Anti-pattern:** an LLM step for a deterministic artifact with repair code underneath — if the generator already exists, remove the LLM.
  - **The boundary as implemented (`agents/codegen.py`, golden-tested vs v1.0.0):** LLM-owned = the pipeline script (pandas or Spark), the SQL DDL (Trino / Unity Catalog), the pipeline Terraform, and all diagnosis. CODE-OWNED (rendered from config, validated, never LLM-emitted) = `requirements.txt`, `dashboards/monitoring_specs.json`, the Lakeview dashboard, `Dockerfile`, all six K8s manifests, and the deploy workflow (all four clouds). The standards for code-owned artifacts stay in the KB as the generator's SPEC + the Medic's reference — **a rule change there requires the matching render change in the same commit** (each carries a GENERATION: CODE-OWNED banner). There is deliberately NO LLM fallback when codegen fails — the Medic patches the file; the permanent fix is the generator.
...
**CI polling (medic):** the verification fetch is **deterministic in Python**, NOT left to the LLM. On every verification-phase medic entry (`infra_status=="completed"` + a `last_push_sha`), the node itself calls `fetch_github_action_logs` and decides: green → `mission_status="verified"`, PENDING → re-poll, real failure → falls through to the LLM for diagnosis/`request_fix`. This is because gpt-4o-mini would SKIP the re-fetch on a re-poll turn (it followed the prompt's "tell the user you are waiting and finish your turn"), so the poll loop stalled and the supervisor's LLM fallback FINISHed with `mission_status` **unset** (MissionFailedError) on an otherwise-successful deploy — masked locally (deploy usually green by the first poll) but surfaced when the agent runs IN CI and the deploy workflow is still QUEUED. Poll cadence: 5 retries with exponential backoff (30s → 60s → 120s → 240s → 300s). After attempt 5 the run is TERMINAL: medic sets `fix_loop_escalated` + `mission_status="ci_unverified"`, supervisor routes FINISH deterministically (never the LLM fallback — that used to re-enter polling with a fresh counter and could sleep for hours toward the recursion limit).
...
> **Enforcement note (post-codegen):** the rules for K8s manifests, the Dockerfile, the
> Grafana/Lakeview dashboards, `requirements.txt` and the deploy workflow below are now
> **enforced by `agents/codegen.py` renders** (golden-tested) — they remain here as the
> spec and the Medic's reference; changing one means changing the matching render in the
> same commit. The **Python pipeline script** rules are still LLM guidance (the script is
> LLM-owned).

**Python pipeline scripts:**
- `storage_options=dict()` is mandatory in every `to_parquet()` call (use `dict()`, not `{}`, so the model can't double-brace it into `{{}}`).
- Trino partition registration inlines the catalog `hive`, the schema AND the table as **string literals** in a plain (non-f) `cursor.execute` — never `catalog`/`schema`/`table` variables. The model otherwise fills the value into BOTH the assignment and the string, leaving the variable unused (ruff F841) and the f-string placeholder-less (ruff F541). `sync_partition_metadata` takes **EXACTLY 3 args** `('<schema>', '<table>', 'ADD')` — the catalog `hive` lives ONLY in the `hive.system.` prefix, never in the args (neither dotted `'hive.sales_eu'` → "Table not found", nor as a separate 4th arg `'hive','schema',…` → "Cannot cast varchar to boolean"). Guaranteed deterministically (`_fix_sync_partition_schema_arg`, normalises both, see above).
...
- **Coerce-then-compare is TWO statements** — assign the coerced column back FIRST, then compare/clamp on a separate line. Chaining a comparison onto the coerce (`pd.to_numeric(chunk[col], …).where(chunk[col] >= 0, …)` for a "replace negative/non-numeric with 0" rule) evaluates the `.where`/comparison against the ORIGINAL `str`/text column (not yet assigned) → `TypeError: Invalid comparison between dtype=str and int` at RUNTIME — it PASSES the `.astype(float)` check, so `validate_generated_code` flags this chained form as a **separate** guard (bit `global_marketing` 2026-06-18). Correct: `chunk[col] = pd.to_numeric(chunk[col], errors='coerce')` then `chunk[col] = chunk[col].fillna(0).clip(lower=0)`.
- **Schema column cross-check (KeyError guard):** `read_data_schema` caches the source table's real column names (`_LAST_SCHEMA_CACHE` in `agents/tools.py`); `validate_generated_code` flags any `chunk['<col>']` the script READS that is neither a source column nor one the script CREATED on an earlier line — catches the LLM using a business rule's `target_criteria` value literally (e.g. `'campaign'` instead of resolving to `'campaign_id'`), a runtime `KeyError`. **Fail-open** (skipped unless the cached table matches the script's `SELECT`, so it never false-positives on the validated runs) and **order-aware** (a derived accumulator like `chunk['is_suspicious'] = chunk['is_suspicious'] | …` is not flagged). The architect must pass `read_data_schema`'s `db_type` from `DATA_SOURCE.db_type` (`architect.md`): the tool **defaults to `postgres`**, so omitting it on a MySQL source connects psycopg2 to port 3306 → `received invalid response to SSL negotiation` (which the medic may mis-route to infra).
...
- Am I fixing the standard/prompt (root cause) or patching the output (symptom)? For a CODE-OWNED artifact (codegen render): standard + render change together, in the same commit.
...
**Never edit generated artifacts to get a quick result.** `scripts/pipe_*.py`, `k8s/`, `sql/`, `dashboards/`, `terraform/`, `.github/workflows/<pipeline>_pipeline.yml`, `Dockerfile`, `requirements.txt` are OUTPUTS — fix the standard/prompt (LLM-owned artifacts) or the standard + `agents/codegen.py` render (code-owned artifacts) and let the next agent run regenerate them. (`scripts/seed_chaos.py`, `scripts/ingest_to_pinecone.py`, `scripts/export_bootstrap_outputs.py` are agent-side utilities — normal code.) The only exception is repo-infrastructure the agent does not generate (e.g. `run_agent.yml`, `bootstrap/`).
