...
- Trino partition registration inlines the catalog `hive`, the schema AND the table as **string literals** in a plain (non-f) `cursor.execute` — never `catalog`/`schema`/`table` variables. The model otherwise fills the value into BOTH the assignment and the string, leaving the variable unused (ruff F841) and the f-string placeholder-less (ruff F541).
