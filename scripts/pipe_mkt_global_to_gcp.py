...
def run():
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            # 1. spend_integrity
            _before = len(chunk)
            chunk.loc[chunk['ad_spend'].astype(float) < 0, 'ad_spend'] = 0.0
            ...
            rejected_by_reason['campaign_standardization'] = _before - len(chunk)
            ...
            int_cols = [c for c in chunk.select_dtypes(include='float64').columns
                        if any(kw in c.lower() for kw in ['clicks', 'impressions'])]
            for col in int_cols:
