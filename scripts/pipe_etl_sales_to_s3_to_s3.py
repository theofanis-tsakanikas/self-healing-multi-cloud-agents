...
def run():
    logging.info("Pipeline starting: pipe_etl_sales_to_s3_to_s3")  # ← MUST be the very first line
    ...
    for _attempt in range(5):
        try:
            cursor.execute("CALL hive.system.sync_partition_metadata('hive.sales_eu', 'pipe_etl_sales_to_s3_to_s3', 'ADD')")
            cursor.fetchall()
            break
