...
def run():
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            _mask = ~chunk['email_address'].str.contains('@', na=False)
    ...
    Gauge('pipeline_rows_rejected_total', 'Rows removed by DROP_RECORD / EXCLUDE_AND_LOG rules',
          ['project_id', 'cloud_provider'], registry=registry) \
        .labels(project_id=project_id, cloud_provider=cloud_provider).set(sum(rejected_by_reason.values()))
    Gauge('pipeline_duration_seconds', 'Wall-clock duration of the extract-transform-write phase',
          ['project_id', 'cloud_provider'], registry=registry) \
        .labels(project_id=project_id, cloud_provider=cloud_provider).set(time.time() - start_time)
    ...
    logging.info(
        f"Metrics pushed: rows={total_rows}, rejected={sum(rejected_by_reason.values())}, "
        f"by_reason={rejected_by_reason}"
    )
