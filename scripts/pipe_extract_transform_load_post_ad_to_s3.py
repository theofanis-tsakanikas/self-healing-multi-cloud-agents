...
def run():
    logging.info("Pipeline starting: pipe_extract_transform_load_post_ad_to_s3")
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            # 3b. Business rules
            # monetary_integrity: DROP_RECORD
            chunk['unit_price'] = pd.to_numeric(chunk['unit_price'], errors='coerce')
            _before = len(chunk)
            chunk = chunk[chunk['unit_price'] > 0.0]
            rejected_by_reason['monetary_integrity'] = _before - len(chunk)

            # temporal_validity: EXCLUDE_AND_LOG
            _before = len(chunk)
    ...
    for _attempt in range(5):
        try:
            cursor.execute("CALL hive.system.sync_partition_metadata('sales_eu', 'pipe_extract_transform_load_post_ad_to_s3', 'ADD')")
            cursor.fetchall()
            break
