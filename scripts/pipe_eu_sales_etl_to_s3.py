...
def run():
    logging.info("Pipeline starting: pipe_eu_sales_etl_to_s3")  # ← MUST be the very first line
    ...
    for _attempt in range(5):
        try:
            cursor.execute("CALL hive.system.sync_partition_metadata('sales_eu', 'pipe_eu_sales_etl_to_s3', 'ADD')")
            cursor.fetchall()
            break
