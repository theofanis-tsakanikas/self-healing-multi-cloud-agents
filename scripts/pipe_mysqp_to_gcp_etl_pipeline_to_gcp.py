...
def run():
    logging.info("Pipeline starting: pipe_mysqp_to_gcp_etl_pipeline_to_gcp")  # ← MUST be the very first line
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            # 3b. Business rules
            # Rule 1: Replace negative or non-numeric values with 0 for ad_spend
            ...
            # Rule 2: Exclude future-dated events
            _before = len(chunk)
            ...
            # Rule 5: Flag impossible engagement
            chunk['is_suspicious'] = (chunk['clicks'] > chunk['impressions'])

            # Rule 6: Flag extreme click outliers
    ...
    cursor = conn.cursor()
    cursor.execute("CALL hive.system.sync_partition_metadata('marketing_global', 'pipe_mysqp_to_gcp_etl_pipeline_to_gcp', 'ADD')")
