...
def run():
    logging.info("Pipeline starting: pipe_etl_pipeline_postgres_adls_to_azure")
    ...
    query = "SELECT * FROM raw_us_crm"  # source table

    try:
        engine = create_engine(connection_string)
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            # 3a. Business rules — translate ALL quality_standards from pipeline config.
            ...
            # Rule 2: At least one contact channel
            _before = len(chunk)
            ...
            # Rule 3: Flag duplicates
            chunk['is_suspicious'] = chunk.duplicated(subset=['cust_id', 'full_name'], keep=False)
    ...
    cursor = conn.cursor()
    cursor.execute("CALL hive.system.sync_partition_metadata('crm_us', 'pipe_etl_pipeline_postgres_adls_to_azure', 'ADD')")
