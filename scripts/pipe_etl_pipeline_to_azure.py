...
def run():
    logging.info("Pipeline starting: pipe_etl_pipeline_to_azure")
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            # 1. Remove records with invalid emails
            _before = len(chunk)
            ...
            # 2. Ensure at least one contact channel
            _before = len(chunk)
    ...
    cursor = conn.cursor()
    cursor.execute("CALL hive.system.sync_partition_metadata('crm_us', 'pipe_etl_pipeline_to_azure', 'ADD')")
