...
def run():
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            _before = len(chunk)
            chunk['order_date'] = pd.to_datetime(chunk['order_date'], errors='coerce')
            chunk = chunk[chunk['order_date'] <= pd.Timestamp.now()]  # temporal_validity
            if (_before - len(chunk)) > 0:
                logging.warning(f"Excluded {_before - len(chunk)} future-dated rows (temporal_validity).")
