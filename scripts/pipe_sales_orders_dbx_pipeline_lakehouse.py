...
def run():
    logging.info("Databricks pipeline starting: pipe_sales_orders_dbx_pipeline_lakehouse")
    ...
    table = f"{catalog}.{schema}.pipe_sales_orders_dbx_pipeline_lakehouse"
    audit_table = f"{catalog}.{schema}.pipe_sales_orders_dbx_pipeline_lakehouse_audit"
    ...
    df = df.withColumn("currency", F.when(~(F.col("currency") == "EUR") & ~(F.col("currency") == "GBP"), "EUR").otherwise(F.col("currency")))
    ...
    # Rule 6: Negative or zero quantity is suspicious.
