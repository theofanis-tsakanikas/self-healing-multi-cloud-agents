...
def run():
    logging.info("Pipeline starting: pipe_etl_postgres_to_adls_to_azure")  # ← MUST be the very first line
    ...
    try:
        ...
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            ...
            # Rule 1: Exclude invalid emails
            _before = len(chunk)
            _mask = ~chunk['email_address'].str.contains('@')
            logging.warning(f"Excluded {_mask.sum()} rows: Invalid email structure.")
            chunk = chunk[~_mask]
            rejected_by_reason['invalid_email'] = rejected_by_reason.get('invalid_email', 0) + (_before - len(chunk))

            # Rule 2: Ensure at least one contact channel
            _before = len(chunk)
            chunk = chunk.dropna(subset=['email_address', 'phone_number'], how='all')
            rejected_by_reason['no_contact_channel'] = rejected_by_reason.get('no_contact_channel', 0) + (_before - len(chunk))

            # Rule 3: Flag suspicious duplicates
            _mask_cust_id = chunk['cust_id'].duplicated() | chunk['full_name'].duplicated()
            chunk['is_suspicious'] = _mask_cust_id
    ...
    cursor = conn.cursor()
    cursor.execute("CALL hive.system.sync_partition_metadata('crm_us', 'pipe_etl_postgres_to_adls_to_azure', 'ADD')")
