...
def run():
    logging.info("Pipeline starting: pipe_etl_from_postgress_to_s3_to_s3")
    ...
    for _attempt in range(5):
        try:
            cursor.execute("CALL hive.system.sync_partition_metadata('hive.sales_eu', 'pipe_etl_from_postgress_to_s3_to_s3', 'ADD')")
            cursor.fetchall()
            break
