...
def run():
    logging.info("Pipeline starting: pipe_mysql_etl_to_gcp_to_gcp")
    ...
    total_rows = 0
    rejected_by_reason = {}
    query = "SELECT * FROM raw_global_marketing"  # MySQL source table

    try:
        engine = create_engine(connection_string)
        for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=1000)):
            # Business rules implementation
            # 1. Ensure ad-spend metrics are non-negative for accounting.
            ...
            # 2. Prevent future-dated events from entering hourly reports.
            ...
            # 3. Ensure every log is attributed to a campaign.
            _before = len(chunk)
            ...
            # 4. Align unknown campaign identifiers to a default bucket.
            chunk['campaign'] = chunk['campaign_id'].where(chunk['campaign_id'].str.match(r'CMP-\d{4}'), other='DEFAULT')

            # 5. Flags impossible engagement (CTR > 100%) for downstream review.
            chunk['is_suspicious'] = (chunk['clicks'] > chunk['impressions'])

            # 6. Flags extreme click outliers that suggest bot traffic or technical glitches.
            chunk['is_suspicious'] = chunk['is_suspicious'] | (chunk['clicks'] >= 1000000)

            # Type casting — cast float64 → Int64 for integer/count/quantity columns
    ...
    cursor = conn.cursor()
    cursor.execute("CALL hive.system.sync_partition_metadata('marketing_global', 'pipe_mysql_etl_to_gcp_to_gcp', 'ADD')")
